...
if(isset($_GET['id']) && $_GET['id']!=0){
    ...
    $modules->data['module_id'] = $_GET['module_id'];
...
$form->addField('title_lt', 	array('title'=>'Pavadinimas LT', 'require'=>1, 'type'=>'text', 'value'=>$modules->data['title_lt'], 'editorship'=>1));
$form->addField('title_en', 	array('title'=>'Pavadinimas EN', 'require'=>1, 'type'=>'text', 'value'=>$modules->data['title_en'], 'editorship'=>1));
$form->addField('column_name', 	array('title'=>'Laukelio vardas', 'require'=>1, 'type'=>'text', 'value'=>$modules->data['column_name'], 'editorship'=>1, 'class_method'=>"object=modules::method=checkExistColumnName::admin_error_msg={$cms_phrases['main']['settings']['module']['column_name_exists']}", 'function'=>"function=valid_table_name::admin_error_msg={$cms_phrases['main']['settings']['module']['not_valid_column_name']}"));
...
$form->addField('require', 		array('title'=>'Privalomas', 'require'=>0, 'type'=>'checkbox', 'value'=>$modules->data['require'], 'editorship'=>1));
$form->addField('list', 		array('title'=>'Rodomas sąraše', 'require'=>0, 'type'=>'checkbox', 'value'=>$modules->data['list'], 'editorship'=>1));
$form->addField('multilng', 	array('title'=>'Daugiakalbis', 'require'=>0, 'type'=>'checkbox', 'value'=>$modules->data['multilng'], 'editorship'=>1));

if($_SESSION['admin']['permission']==1){

	$form->addField('editorship', 	array('title'=>'Redaguojamas', 'require'=>0, 'type'=>'checkbox', 'value'=>$modules->data['editorship'], 'editorship'=>1));
	...
	$form->addField('super_user', 	array('title'=>'Tik superadminui', 'require'=>0, 'type'=>'checkbox', 'value'=>$modules->data['superuser'], 'editorship'=>1));
...
if(!empty($_POST)){
    ...
    if($form->error!=1){
		...
		redirect("main.php?content=settings&page=edit_column&module_id={$modules->data['module_id']}&id=$id");
    }
