...
if($modules->data['isNew'] != 1){
	if($_SESSION['admin']['permission']==1){
		...
		$form->addField('area_html', 	array('title'=>$cms_phrases['main']['settings']['module']['area_html'], 'require'=>0, 'type'=>'html', 'value'=>$modules->data['area_html'], 'editorship'=>1, 'htmlspecialchars'=>1, 'list_values'=>array('mode'=>'block', 'toolbar'=>'Form', 'height'=>'350')));
		$form->editField('no_standart_tpl', array('extra_params'=>"onclick=\"javascript: getDefaultModuleTemplate(this, this.form.elements['field_html'], {$modules->data['id']});\""));
	}
}
...
if($_SESSION['admin']['permission']==1){
	...
	$form->addField('mod_pages', 	array('title'=>$cms_phrases['main']['settings']['module']['mod_pages'], 		'require'=>0, 'type'=>'checkbox', 	'value'=>$modules->data['mod_pages'], 'editorship'=>1	));
}
...
if(!empty($_POST)){
    if($_POST['action']=='save'){
	    ...
	    if($form->error!=1){
	        ...
	        redirect("main.php?content=settings&page=edit_module&id=$id");
	    }
    }
}
