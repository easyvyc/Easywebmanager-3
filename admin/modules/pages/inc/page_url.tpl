...
<div id="area_id_{elm.name}" class="formElementsField{block elm.edited} edited{-block elm.edited}">
	...
	<div class="e">
		...
		<input type='text' name='{elm.name}' value='{elm.value}' class='{style}_{elm.elm_type}{block elm.editorship no} readonly{-block elm.editorship no}' onchange="javascript: setEdited('{elm.name}');" {block elm.editorship no}readonly{-block elm.editorship no} ondblclick="javascript: setReadonly(this, document.getElementById('gen_page_url_auto_id'), false, '{style}', '{elm.elm_type}'); " />

		<input type="checkbox" {block elm.editorship no}checked{-block elm.editorship no} value="1" name="generate_url" id="gen_page_url_auto_id" onclick="javascript: setReadonly(this.form.elements['{elm.name}'], this, this.checked, '{style}', '{elm.elm_type}');" style="vertical-align:middle;" /> <label for="gen_page_url_auto_id">Auto URL</label>
