...
if(!isset($_GET['file'])){
	...
	$csv_data = csv_to_arr(UPLOADDIR."csv/".$_GET['file'], $_GET['separator'], $_GET['encoding'], $_GET['sep_title']);
	//pae($csv_data);
	foreach($csv_data['cols'] as $key=>$val){
		$csv_data['cols'][$key]['short'] = (strlen($val['title'])>17?mb_substr($val['title'], 0, 14, "UTF-8")."...":$val['title']);
	}
	$easy_tpl->setFastLoop('columns', $csv_data['cols']);
...
if(!empty($_POST)){
	...
	if(isset($_POST['action']) && $_POST['action']=='import'){
	    ...
	    if($form->error!=1){
			...
			if(file_exists(UPLOADDIR.$form->elements['file']['value']) && rename(UPLOADDIR.$form->elements['file']['value'], UPLOADDIR."csv/".$form->elements['file']['value'])){
...
//			    }
	...
	if(isset($_POST['action']) && $_POST['action']=='data'){
		...
		$csv_data = csv_to_arr(UPLOADDIR."csv/".$_POST['file'], $_POST['separator'], $_POST['encoding'], $_POST['sep_title']);
		$record->insertListItems($csv_data['data'], $_GET['id']);
		unlink(UPLOADDIR."csv/".$_GET['file']);
