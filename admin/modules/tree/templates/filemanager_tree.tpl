<ul class="tree" style="display:{block data.display no}none{-block data.display no}{block data.display}block{-block data.display};" id="main_{data.id}_{get.nm}">
	...
	<li class="item mod_{mod.id}_{get.nm}" id="item_{items.id}_{get.nm}" rel="parent_{items.parent_id}">
		...
		<div id="contextmenu_{items.id}_{get.nm}" class="contextMenu">
			...
			<div class="text">
			{loop items.context}
				<div><a onclick="{items.context.action}"><img src="images/{items.context.img}.gif" alt="" class="vam" border="0" /> {items.context.title}</a></div>
			{-loop items.context}
			</div>
		</div>
		{-block items.context_menu}
		...
		<a onclick="javascript: parent.frmResourcesList.location='main.php?content=filemanager&page=files&path={items.path}'; "><img src="images/tree/{items.img}.gif" class="vam" alt="{items.title}" /></a> 
		{-block get.checkbox no}
		{block get.checkbox}
		...
		{-block get.checkbox}
		<a onclick="javascript: parent.frmResourcesList.location='main.php?content=filemanager&page=files&path={items.path}';" id="link_{items.id}_{get.nm}" {block items.a}class="active"{-block items.a}>{items.title} {block items.sub_count}[{items.sub_count}]{-block items.sub_count}</a>
		<input type="text" id="edit_{items.id}_{get.nm}" value="{items.title}" style="display:none;" class="edittreeitem" onkeypress="javascript: changeTreeItem('{items.path}', '{items.id}', this, event);" />
