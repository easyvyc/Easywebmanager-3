<div id="area_id_{elm.name}" class="formElementsField{block elm.edited} edited{-block elm.edited}">
	...
	<table border="0" id="img_table_{elm.name}" {block is_img no}style="display:none;"{-block is_img no}>
		<tr>
			<td>
			{block data.id.value}
			<img src='file.php?module={get.module}&column={elm.name}&id={data.id.value}&w=200&h=200&t=0' border='0' id="img_src_{elm.name}" onclick="javascript: window.open('file.php?module={get.module}&column={elm.name}&id={data.id.value}&w=800&h=800&t=0');" onload="javascript: changeImageDisplaySize(this);" style="cursor:pointer;">
			{-block data.id.value}
			{block data.id.value no}
