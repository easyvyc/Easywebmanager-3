...
$xml = '<Pie title="'.$cms_phrases['main']['stat']['visitors_by_keyword'].'">';

$sql = "SELECT COUNT(id) AS cnt, keyword AS title FROM {$configFile->variable['sb_stat_visitor']} " .
		"WHERE visit_time >= '{$_GET['from_date']}' AND visit_time < '{$_GET['to_date']}' AND robot!=1 AND keyword!='' " .
		"GROUP BY keyword " .
		"ORDER BY cnt DESC";
